...
/*   Updated: 2025/11/12 18:40:11 by ravazque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					print_exec_error(char *cmd, int error_type,
							int is_path);
void					handle_no_path(char **argv, char *cd_home);
void					ft_execve(char **argv, char **envp, char ***env_ptr,
							char *cd_home);
void					execute_child_process(t_mini *mini, t_cmd *cmd,
							t_exec *exec, int idx);
...
int						collect_lines(char *delim, t_mini *mini, int expand,
							char ***lines);
...
int						add_expanded_tokens(char **result, int *j,
							t_token *tok, t_mini *mini);
...
char					*process_char_exp(char *res, const char *s, int *i,
							t_normi_ctx *ctx);
...
t_redir					*mk_redir(const char *tgt, const char *op,
							t_token *tgt_tok);
...
int						flush_part(char **bf, int sq, int dq,
							t_token_part **parts);
char					*join_parts(t_token_part *parts);
t_token					*mk_tok_from_parts(t_token_part *parts);
int						append_char(char **dst, char c);
int						read_quoted(const char *in, size_t *i, char q,
							char **bf);
int						is_operator_char(char c);
void					cleanup_tok_data(char **bf, t_token_part **p,
							t_cmd **cmd);
int						add_tok_to_cmd(t_token *new, t_cmd *cmd);
int						finalize_tok(char **bf, t_token_part **tp, t_cmd *cmd);
int						handle_operator(const char *in, size_t *i,
							t_normi_ctx *ctx);
int						handle_space(const char *in, size_t *i,
							t_normi_ctx *ctx);
int						handle_quote(const char *in, size_t *i,
							t_normi_ctx *ctx);
int						handle_regular(const char *in, size_t *i,
							t_normi_ctx *ctx);
...
void					init_mini(t_mini *mini, int argc, char *argv[],
							char *envp[]);
